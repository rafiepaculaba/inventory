...
function toDecimal(original){temp=Math.round(original*100)/100;if((temp*100)%100==0)
...
var emailArr=emailStr.split(/[,;]/);for(var i=0;i<emailArr.length;i++){if(!/^\s*[\w.%+\-]+@([A-Z0-9-]+\.)*[A-Z0-9-]+\.[A-Z]{2,4}\s*$/i.test(emailArr[i])&&!/^[\w\s.%+\-]*<[A-Z0-9._%+\-]+?@([A-Z0-9-]+\.)*[A-Z0-9-]+\.[A-Z]{2,4}>\s*$/i.test(emailArr[i]))
return false;}
return true;}
...
return false
return true}
function inRange(value,min,max){value=""+value;value=value.replace(/,/g,'.');return value>=min&&value<=max;}
